...
def get_users_dict():
    ...
    users_dict = []
    for user in users:
        user_dict = model_to_dict(user, exclude=["discord_servers"])
...
def add_user(info):
    ...
    for k, v in info.items():
        if k in ["id", "discord_servers", "added", "username", 'password']:
            continue
...
def edit_user_from_dict(data):
    ...
    for k, v in data.items():
        if hasattr(user, k):
            ...
            if k not in ["id", "discord_servers", "added", "username"]:
                setattr(user, k, v)
